(function(Scratch) {
    ...
    class SpikePrimeBridge {
        constructor() {
            ...
            // Gestures
            this.gestures = { tapped: false, doubletapped: false, shake: false, freefall: false };
        ...
        async _connect(url) {
            return new Promise((resolve, reject) => {
                ...
                console.log(`🔌 Connecting to ${wsUrl}... (attempt ${this.reconnectAttempts + 1})`);
...
`;
        ...
        _parseJSONResponse(response) {
            if (response.m === 0) {
                ...
                if (gestures.includes(response.p)) {
                    ...
                    setTimeout(() => { this.gestures[response.p] = false; }, 100);
                }
            }
        ...
        _parseRawData(text) {
            if (text.startsWith('SENSORS:')) {
                ...
                if (this.gestures.hasOwnProperty(gesture)) {
                    ...
                    setTimeout(() => { this.gestures[gesture] = false; }, 100);
                }
                ...
                if (this.replOutput.length > 1000) {
                    this.replOutput = this.replOutput.substring(this.replOutput.length - 1000);
                }
            }
        }
        ...
        async motorRunFor(args) {
            ...
            if (unit === 'rotations') {
                ...
                return this._sendPythonCommand(`import hub; hub.port.${port}.motor.run_for_degrees(${degrees}, ${speed})`);
            } else if (unit === 'degrees') {
                return this._sendPythonCommand(`import hub; hub.port.${port}.motor.run_for_degrees(${Math.floor(value)}, ${speed})`);
            } else if (unit === 'seconds') {
                const ms = Math.floor(value * 1000);
                return this._sendPythonCommand(`import hub; hub.port.${port}.motor.run_for_time(${ms}, ${speed})`);
            }
        }
        ...
        async motorStart(args) {
            ...
            const direction = Cast.toNumber(args.DIRECTION);
            const speed = this.motorSettings[port].speed * direction;
            return this._sendPythonCommand(`import hub; hub.port.${port}.motor.pwm(${Math.round(speed)})`);
        }

        async motorStop(args) {
            const port = Cast.toString(args.PORT).trim().toUpperCase();
            return this._sendPythonCommand(`import hub; hub.port.${port}.motor.stop()`);
        }
        ...
        async displayText(args) {
            ...
            return this._sendPythonCommand(`import hub; hub.display.show("${text.replace(/"/g, '\\"')}")`);
        }

        async displayImage(args) {
            const matrix = Cast.toString(args.MATRIX);
            const symbol = (matrix.replace(/\D/g, '') + '0'.repeat(25)).slice(0, 25);
            const image = symbol.replace(/1/g, '9').replace(/0/g, '_').match(/.{5}/g).join(':');
            return this._sendPythonCommand(`import hub; hub.display.show(hub.Image("${image}"))`);
        }
        ...
        async setPixel(args) {
            ...
            return this._sendPythonCommand(`import hub; hub.display.pixel(${x}, ${y}, ${brightness})`);
        }
        ...
        async playBeep(args) {
            ...
            return this._sendPythonCommand(`import hub; hub.sound.beep(${freq}, ${duration}, hub.sound.SOUND_SIN)`);
        }
    ...
    class SpikePrimeBridgeExtension {
                ...
                blocks: [
                    {
                        ...
                        text: '🔌 connect to [URL]',
                        arguments: {
                            URL: { type: Scratch.ArgumentType.STRING, defaultValue: 'localhost:8081' }
                        }
                    },
                    {
                    ...
                    '---',
                    {
                        ...
                        arguments: {
                            PORT: { type: Scratch.ArgumentType.STRING, menu: 'PORT', defaultValue: 'A' },
                            DIRECTION: { type: Scratch.ArgumentType.NUMBER, menu: 'DIRECTION', defaultValue: 1 },
                            VALUE: { type: Scratch.ArgumentType.NUMBER, defaultValue: 1 },
                            UNIT: { type: Scratch.ArgumentType.STRING, menu: 'MOTOR_UNIT', defaultValue: 'rotations' }
                        }
                    },
                    {
                        opcode: 'motorStart',
                    ...
                    },
                    {
                        ...
                        text: 'turn on [MATRIX]',
                        arguments: {
                            MATRIX: { type: Scratch.ArgumentType.MATRIX, defaultValue: '1101111011000001000101110' }
                        }
                    },
                    {
                        ...
                        arguments: {
                            PATTERN: { type: Scratch.ArgumentType.STRING, menu: 'DISPLAY_PATTERN', defaultValue: 'heart' }
                        }
                    },
                    {
                        opcode: 'displayClear',
                    ...
                    },
                    {
                        ...
                        arguments: {
                            ...
                            BRIGHTNESS: { type: Scratch.ArgumentType.NUMBER, defaultValue: 100 }
                        }
                    },
                    '---',
                    {
                        ...
                        text: '[AXIS] angle',
                        arguments: {
                            AXIS: { type: Scratch.ArgumentType.STRING, menu: 'AXIS', defaultValue: 'pitch' }
                        }
                    },
                    {
                        ...
                        arguments: {
                            AXIS: { type: Scratch.ArgumentType.STRING, menu: 'AXIS_XYZ', defaultValue: 'x' }
                        }
                    },
                    {
                        opcode: 'resetYaw',
                    ...
                    '---',
                    {
                        ...
                        arguments: {
                            GESTURE: { type: Scratch.ArgumentType.STRING, menu: 'GESTURE', defaultValue: 'tapped' }
                        }
                    },
                    '---',
                    {
                        ...
                        arguments: {
                            FREQUENCY: { type: Scratch.ArgumentType.NUMBER, defaultValue: 440 },
                    ...
                    '---',
                    {
                        ...
                        arguments: {
                            CODE: { type: Scratch.ArgumentType.STRING, defaultValue: 'print("Hello")' }
                        }
                    },
                    {
        ...
        connectHub(args) { return this.peripheral.connect(args.URL); }
        disconnectHub() { return this.peripheral.disconnect(); }
        isConnected() { return this.peripheral.isConnected(); }
        motorRunFor(args) { return this.peripheral.motorRunFor(args); }
        motorStart(args) { return this.peripheral.motorStart(args); }
        motorStop(args) { return this.peripheral.motorStop(args); }
        motorSetSpeed(args) { return this.peripheral.motorSetSpeed(args); }
        getPosition(args) { return this.peripheral.getPosition(args); }
        displayText(args) { return this.peripheral.displayText(args); }
        displayImage(args) { return this.peripheral.displayImage(args); }
        displayPattern(args) { return this.peripheral.displayPattern(args); }
        displayClear() { return this.peripheral.displayClear(); }
        setPixel(args) { return this.peripheral.setPixel(args); }
        getAngle(args) { return this.peripheral.getAngle(args); }
        getAcceleration(args) { return this.peripheral.getAcceleration(args); }
        resetYaw() { return this.peripheral.resetYaw(); }
        getDistance(args) { return this.peripheral.getDistance(args); }
        getColor(args) { return this.peripheral.getColor(args); }
        getForce(args) { return this.peripheral.getForce(args); }
        isForceSensorPressed(args) { return this.peripheral.isForceSensorPressed(args); }
        whenGesture(args) { return this.peripheral.whenGesture(args); }
        playBeep(args) { return this.peripheral.playBeep(args); }
        stopSound() { return this.peripheral.stopSound(); }
        getBatteryLevel() { return this.peripheral.getBatteryLevel(); }
        getTimer() { return this.peripheral.getTimer(); }
        resetTimer() { return this.peripheral.resetTimer(); }
        runPythonCommand(args) { return this.peripheral.runPythonCommand(args); }
        getReplOutput() { return this.peripheral.getReplOutput(); }
        clearReplOutput() { return this.peripheral.clearReplOutput(); }
    }
