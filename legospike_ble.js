    ...
    class SpikePrimeExtension {
            ...
            return {
                ...
                blocks: [
                    ...
                    '---',
                    {
                        ...
                        arguments: {
                            PORT: { type: Scratch.ArgumentType.STRING, menu: 'PORT', defaultValue: 'C' },
                    ...
                    '---',
                    {
                        ...
                        arguments: {
                            PORT: { type: Scratch.ArgumentType.STRING, menu: 'PORT', defaultValue: 'B' }
                        }
                    },
                    {
                        ...
                        arguments: {
                            PORT: { type: Scratch.ArgumentType.STRING, menu: 'PORT', defaultValue: 'D' }
                        }
                    },
                    {
                    ...
                    },
                    {
                        ...
                        arguments: {
                            PORT: { type: Scratch.ArgumentType.STRING, menu: 'PORT', defaultValue: 'E' }
                        }
                    },
                    '---',
                    {
                        ...
                        arguments: {
                            AXIS: { type: Scratch.ArgumentType.STRING, menu: 'ORIENTATION_AXIS', defaultValue: 'yaw' }
                        }
                    },
                    {
                        ...
                        arguments: {
                            AXIS: { type: Scratch.ArgumentType.STRING, menu: 'XYZ_AXIS', defaultValue: 'x' }
                        }
                    },
                    {
                        opcode: 'getFaceUp',
        ...
        async connectHub() {
            ...
            try {
                ...
                const service = await this.server.getPrimaryService(HUB_CONSTANTS.SERVICE_UUID);
                console.log('✅ [SPIKE Prime] Service found');

                console.log('🔍 [SPIKE Prime] Getting characteristics...');
                this.rxCharacteristic = await service.getCharacteristic(HUB_CONSTANTS.RX_UUID);
                this.txCharacteristic = await service.getCharacteristic(HUB_CONSTANTS.TX_UUID);
        ...
        _onMessage(event) {
            try {
                ...
                if (DEBUG) console.log('📨 [SPIKE Prime] Message type:', '0x' + msgType.toString(16));
                ...
                switch (msgType) {
                    ...
                    case HUB_CONSTANTS.MSG_DEVICE_NOTIFICATION_RESPONSE:
                        if (DEBUG) console.log('✅ [SPIKE Prime] Notification request acknowledged');
                        break;
                    default:
                        if (DEBUG) console.warn('❓ [SPIKE Prime] Unknown message type:', '0x' + msgType.toString(16));
                        break;
                }
            } catch (error) {
        ...
        _handleInfoResponse(data) {
            ...
            console.log('ℹ️  [SPIKE Prime] Hub info - Packet size:', this.maxPacketSize, 'Chunk size:', this.maxChunkSize);
        }
            ...
            while (offset < payload.length) {
                ...
                switch (deviceType) {
                    ...
                    case HUB_CONSTANTS.DEVICE_IMU:
                        if (remaining >= 21) {
                            const imuView = new DataView(payload.buffer, payload.byteOffset + offset);
                            ...
                            if (DEBUG) console.log('🧭 IMU:', this.imu.yaw, this.imu.pitch, this.imu.roll);
                            offset += 21;
                        } else offset = payload.length;
                        break;
                    ...
                    case HUB_CONSTANTS.DEVICE_MOTOR:
                        if (remaining >= 11) {
                            ...
                            if (portName) {
                                const motorView = new DataView(payload.buffer, payload.byteOffset + offset);
                                this.ports[portName].value.position = motorView.getInt32(8, true);
                                if (DEBUG) console.log('⚙️  Motor', portName, 'position:', this.ports[portName].value.position);
                            }
                            offset += 11;
                        } else offset = payload.length;
                        break;
                    ...
                    case HUB_CONSTANTS.DEVICE_FORCE_SENSOR:
                        if (remaining >= 4) {
                            ...
                            if (portName) {
                                ...
                                if (DEBUG) console.log('👆 Force', portName + ':', this.ports[portName].value.force + '%');
                            }
                            offset += 4;
                        } else offset = payload.length;
                        break;
                    ...
                    case HUB_CONSTANTS.DEVICE_COLOR_SENSOR:
                        if (remaining >= 9) {
                            ...
                            if (portName) {
                                ...
                                if (DEBUG) console.log('🎨 Color', portName + ':', this.ports[portName].value.color);
                            }
                            offset += 9;
                        } else offset = payload.length;
                        break;
                    ...
                    case HUB_CONSTANTS.DEVICE_DISTANCE_SENSOR:
                        if (remaining >= 4) {
                            ...
                            const portName = PORT_ID_TO_NAME[portId];
                            if (portName) {
                                const distView = new DataView(payload.buffer, payload.byteOffset + offset);
                                this.ports[portName].value.distance = distView.getInt16(2, true);
                                if (DEBUG) console.log('📏 Distance', portName + ':', this.ports[portName].value.distance + 'mm');
                            }
                            offset += 4;
                        } else offset = payload.length;
                        break;
                    ...
                    default:
                        if (DEBUG) console.warn('❓ Unknown device type:', '0x' + deviceType.toString(16));
        ...
        async stopMotor(args) {
            ...
            console.log('🛑 [SPIKE Prime] Stop motor', args.PORT, 'action:', args.ACTION);
        ...
        async setLightMatrixPixel(args) {
            ...
            const brightness = Math.round(Math.max(0, Math.min(100, args.BRIGHTNESS)) / 10);
            
            console.log('💡 [SPIKE Prime] Light', args.PORT, 'pixel (' + x + ',' + y + ') brightness:', args.BRIGHTNESS + '%');
        ...
        getColor(args) {
            ...
            const colors = {
                0: 'black', 1: 'magenta', 2: 'purple', 3: 'blue',
                4: 'azure', 5: 'turquoise', 6: 'green', 7: 'yellow',
                8: 'orange', 9: 'red', 10: 'white', 255: 'unknown'
            };
        ...
        _cobsEncode(data) {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (byte <= 2) {
                    ...
                    const block_offset = (buffer.length - code_index) + HUB_CONSTANTS.COBS_CODE_OFFSET;
