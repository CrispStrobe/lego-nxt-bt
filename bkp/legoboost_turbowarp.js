(function (Scratch) {
  ...
  class BoostBridgeExtension {
    ...
    whenColor(args) {
      if (args.COLOR === BoostColor.ANY) {
        return (
          this.peripheral.color !== BoostColor.NONE &&
    ...
    _isTilted(direction) {
      switch (direction) {
        case BoostTiltDirection.ANY:
          return (
            Math.abs(this.peripheral.tiltX) >=
              BoostBridgeExtension.TILT_THRESHOLD ||
            Math.abs(this.peripheral.tiltY) >=
              BoostBridgeExtension.TILT_THRESHOLD
          );
        default:
          return (
            this._getTiltAngle(direction) >= BoostBridgeExtension.TILT_THRESHOLD
          );
      }
    }
    ...
    _getTiltAngle(direction) {
      switch (direction) {
        case BoostTiltDirection.UP:
          return this.peripheral.tiltY > 90
            ? 256 - this.peripheral.tiltY
            : -this.peripheral.tiltY;
        case BoostTiltDirection.DOWN:
          return this.peripheral.tiltY > 90
            ? this.peripheral.tiltY - 256
            : this.peripheral.tiltY;
        case BoostTiltDirection.LEFT:
          return this.peripheral.tiltX > 90
            ? this.peripheral.tiltX - 256
            : this.peripheral.tiltX;
        case BoostTiltDirection.RIGHT:
          return this.peripheral.tiltX > 90
            ? 256 - this.peripheral.tiltX
            : -this.peripheral.tiltX;
        default:
          return 0;
      }
    ...
    whenMotorOverloaded() {
      return (
        this.peripheral.hubStatus.highCurrent ||
        this.peripheral.hubStatus.overPower
      );
    }
  }
