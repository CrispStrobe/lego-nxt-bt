(function (Scratch) {
  ...
  const translations = {
    en: {
      ...
      "legonxt.motorRunRotations": "motor [PORT] power [POWER] for [ROTATIONS] rotations",
      ...
      "legonxt.spriteSetPosition": "set sprite [SPRITE] position to x:[X] y:[Y]",
  ...
  class NXCTranspiler {
    ...
    getFieldValue(block, fieldName) {
        this.logger.debug(`Getting field "${fieldName}" from block ${block.opcode}`);
        ...
        if (block.fields && block.fields[fieldName]) {
            ...
            if ((fieldName === 'BROADCAST_OPTION' || fieldName === 'BROADCAST_INPUT') 
                && this.broadcastMap && this.broadcastMap[value]) {
                const mappedName = this.broadcastMap[value];
                this.logger.success(`  ✓ Mapped broadcast field: ${value} → "${mappedName}"`);
                return mappedName;
            }
        ...
        this.logger.warn(`  ⚠️ Field "${fieldName}" not found in block ${block.opcode}`);
        return null;
    }
    ...
    getInputValue(block, inputName, blocks) {
        this.logger.debug(`Getting input "${inputName}" from block ${block.opcode} (ID: ${block.id})`);
        
        const input = block.inputs[inputName];
        if (!input) {
            this.logger.warn(`❌ Missing input: ${inputName} in block ${block.opcode} (ID: ${block.id})`);
            this.logger.debug(`Available inputs: ${Object.keys(block.inputs).join(', ') || 'none'}`);
            return "0";
        }

        this.logger.debug(`Input type: ${typeof input}, isArray: ${Array.isArray(input)}`);
        ...
        if (typeof input === "object" && !Array.isArray(input)) {
            ...
            if (input.block) {
                ...
                if (refBlock) {
                    this.logger.debug(`Evaluating referenced block: ${refBlock.opcode}`);
                    const result = this.evaluateBlock(refBlock, blocks);
                    this.logger.success(`✓ Resolved "${inputName}" via block reference → ${result}`);
                    return result;
                } else {
                    this.logger.warn(`⚠️ Block reference ${input.block} not found in blocks._blocks`);
                }
            }
                ...
                if (shadowBlock) {
                    ...
                    if (shadowBlock.opcode === "data_variable" || 
                        ...
                        if (this.broadcastMap && this.broadcastMap[varId]) {
                            ...
                            this.logger.warn(`⚠️ Broadcast ID ${varId} not in broadcastMap, using as-is`);
                            return `"${varId}"`;
                        }
                    }
                    ...
                    this.logger.success(`✓ Resolved "${inputName}" via shadow block → ${result}`);
                    return result;
                } else {
            ...
            this.logger.warn(`❌ Could not resolve object-style input "${inputName}"`);
            return "0";
        }

        // ==================== HANDLE ARRAY-STYLE INPUTS (OLD SCRATCH FORMAT) ====================
        if (!Array.isArray(input)) {
            this.logger.error(`❌ Input is neither object nor array for "${inputName}"`);
            return "0";
        }
        ...
        this.logger.debug(`Array-style input: type=${inputType}, data type=${typeof inputData}`);
        ...
        if (inputType === 1 || inputType === 2 || inputType === 3) {
            ...
            if (Array.isArray(inputData)) {
                ...
                this.logger.debug(`Primitive: type=${primitiveType}, value="${primitiveValue}"`);
                ...
                // String/variable primitive (10)
                    ...
                    if (this.broadcastMap && this.broadcastMap[primitiveValue]) {
                        ...
                        this.logger.success(`✓ Mapped broadcast in primitive: ${primitiveValue} → "${name}"`);
                        return `"${name}"`;
                    }
                ...
                // Broadcast primitive (11) - Scratch 3.0 specific
                    ...
                    this.logger.debug(`Broadcast primitive detected: ${primitiveValue}`);
                    ...
                    if (this.broadcastMap && this.broadcastMap[broadcastId]) {
                        ...
                        this.logger.success(`✓ Mapped broadcast primitive: ${broadcastId} → "${mappedName}"`);
                        return `"${mappedName}"`;
                    } else if (broadcastName) {
                        this.logger.success(`✓ Using broadcast name from primitive: "${broadcastName}"`);
                        return `"${broadcastName}"`;
                    } else {
                        this.logger.warn(`⚠️ Broadcast primitive ${broadcastId} not mapped, using ID`);
                        return `"${broadcastId}"`;
                    }
                }
            }
            ...
            if (inputType === 3 && Array.isArray(inputData) && inputData.length >= 2) {
                ...
                if (typeof inputData[0] === "string") {
                    ...
                    if (refBlock) {
                        this.logger.debug(`Evaluating type 3 main block: ${refBlock.opcode}`);
                        const result = this.evaluateBlock(refBlock, blocks);
                ...
                if (Array.isArray(shadowData)) {
                    ...
                    this.logger.debug(`Type 3 shadow: type=${primitiveType}, value="${primitiveValue}"`);
                    ...
                    if (primitiveType === 4 || primitiveType === 5 || 
                        ...
                        if (this.isNumeric(primitiveValue)) {
                            this.logger.success(`✓ Type 3 shadow numeric string: ${primitiveValue}`);
                            return String(primitiveValue);
                        }
                        ...
                        if (this.broadcastMap && this.broadcastMap[primitiveValue]) {
                            const name = this.broadcastMap[primitiveValue];
                            this.logger.success(`✓ Type 3 shadow broadcast: ${primitiveValue} → "${name}"`);
                            return `"${name}"`;
                        }
                        
                        this.logger.success(`✓ Type 3 shadow string: "${primitiveValue}"`);
                        return '"' + primitiveValue + '"';
                    } else if (primitiveType === 11) {
                        // Broadcast in shadow
                        ...
                        if (this.broadcastMap && this.broadcastMap[broadcastId]) {
                            const mappedName = this.broadcastMap[broadcastId];
                            this.logger.success(`✓ Type 3 shadow broadcast: ${broadcastId} → "${mappedName}"`);
                            return `"${mappedName}"`;
                        } else if (broadcastName) {
                            this.logger.success(`✓ Type 3 shadow broadcast name: "${broadcastName}"`);
                            return `"${broadcastName}"`;
                        }
                    }
                }
        ...
        this.logger.error(`❌ Failed to resolve input "${inputName}" - returning "0"`);
        this.logger.debug(`Final input structure:`, JSON.stringify(input, null, 2));
        return "0";
    }
    ...
    evaluateBlock(block, blocks) {
        ...
        // ===== NUMBERS AND TEXT =====
        if (opcode === "math_number" || opcode === "math_whole_number" || 
            opcode === "math_positive_number" || opcode === "math_integer") {
        ...
        // ===== NXT MOTOR REPORTERS =====
        else if (opcode === "legonxt_getMotorPosition" || opcode === "getMotorPosition") {
            let p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            if (p === "0") p = "A"; else if (p === "1") p = "B"; else if (p === "2") p = "C";
            return `MotorRotationCount(OUT_${p})`;
        }
        else if (opcode === "legonxt_getMotorPower" || opcode === "getMotorPower") {
            let p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            if (p === "0") p = "A"; else if (p === "1") p = "B"; else if (p === "2") p = "C";
            return `MotorPower(OUT_${p})`; // 
        }
        else if (opcode === "legonxt_getMotorMode" || opcode === "getMotorMode") {
            let p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            if (p === "0") p = "A"; else if (p === "1") p = "B"; else if (p === "2") p = "C";
            return `MotorMode(OUT_${p})`; // 
        }
        else if (opcode === "legonxt_getMotorRegulationMode" || opcode === "getMotorRegulationMode") {
            let p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            if (p === "0") p = "A"; else if (p === "1") p = "B"; else if (p === "2") p = "C";
            return `MotorRegulation(OUT_${p})`; //  (note: "Regulation" not "RegulationMode")
        }
        else if (opcode === "legonxt_getMotorTurnRatio" || opcode === "getMotorTurnRatio") {
            let p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            if (p === "0") p = "A"; else if (p === "1") p = "B"; else if (p === "2") p = "C";
            return `MotorTurnRatio(OUT_${p})`; // 
        }
        else if (opcode === "legonxt_getMotorRunState" || opcode === "getMotorRunState") {
            let p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            if (p === "0") p = "A"; else if (p === "1") p = "B"; else if (p === "2") p = "C";
            return `MotorRunState(OUT_${p})`; // 
        }
        else if (opcode === "legonxt_getMotorTachoLimit" || opcode === "getMotorTachoLimit") {
            let p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            if (p === "0") p = "A"; else if (p === "1") p = "B"; else if (p === "2") p = "C";
            return `MotorTachoLimit(OUT_${p})`; // 
        }
        else if (opcode === "legonxt_getMotorTachoCount" || opcode === "getMotorTachoCount") {
            let p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            if (p === "0") p = "A"; else if (p === "1") p = "B"; else if (p === "2") p = "C";
            return `MotorTachoCount(OUT_${p})`; // 
        }
        else if (opcode === "legonxt_getMotorBlockTachoCount" || opcode === "getMotorBlockTachoCount") {
            let p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            if (p === "0") p = "A"; else if (p === "1") p = "B"; else if (p === "2") p = "C";
            return `MotorBlockTachoCount(OUT_${p})`; // 
        }
        else if (opcode === "legonxt_getMotorRotationCount" || opcode === "getMotorRotationCount") {
            let p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            if (p === "0") p = "A"; else if (p === "1") p = "B"; else if (p === "2") p = "C";
            return `MotorRotationCount(OUT_${p})`; // 
        }
        ...
        else if (opcode === "legonxt_isTouchPressed" || opcode === "isTouchPressed") {
        ...
        else if (opcode === "legonxt_getLightLevel" || opcode === "getLightLevel") {
            const p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            let n = p.replace("S", "");
            if (n === "0") n = "1";
        ...
        else if (opcode === "legonxt_getSoundLevel" || opcode === "getSoundLevel") {
            const p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            let n = p.replace("S", "");
            if (n === "0") n = "1";
        ...
        // ===== SPRITE STATE REPORTERS =====
            ...
            const sprite = this.getInputValue(block, "SPRITE", blocks).replace(/"/g, "");
        ...
        else if (opcode === "legonxt_spriteGetSize" || opcode === "spriteGetSize") {
            const sprite = this.getInputValue(block, "SPRITE", blocks).replace(/"/g, "");
            const safeName = this.sanitizeName(sprite);
            return `sprite_${safeName}.size`;
        }
        else if (opcode === "legonxt_spriteGetVisible" || opcode === "spriteGetVisible") {
            const sprite = this.getInputValue(block, "SPRITE", blocks).replace(/"/g, "");
            const safeName = this.sanitizeName(sprite);
    ...
    processBlock(block, blocks) {
        ...
        if (opcode === "legonxt_motorOn" || opcode === "motorOn") {
        ...
        else if (opcode === "legonxt_motorRunDegrees" || opcode === "motorRunDegrees") {
            let p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            if (p === "0") p = "A"; else if (p === "1") p = "B"; else if (p === "2") p = "C";
            const power = this.getInputValue(block, "POWER", blocks);
        ...
        else if (opcode === "legonxt_motorRunRotations" || opcode === "motorRunRotations") {
            let p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            if (p === "0") p = "A"; else if (p === "1") p = "B"; else if (p === "2") p = "C";
            const power = this.getInputValue(block, "POWER", blocks);
            ...
            this.addLine(action === "brake" ? `Off(OUT_${p});` : `Float(OUT_${p});`);
        }
        else if (opcode === "legonxt_resetMotorPosition" || opcode === "resetMotorPosition") {
            let p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            if (p === "0") p = "A"; else if (p === "1") p = "B"; else if (p === "2") p = "C";
        ...
        else if (opcode === "legonxt_setupTouchSensorNXT" || opcode === "setupTouchSensorNXT") {
            const p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            let n = p.replace("S", ""); if (n === "0") n = "1";
            this.addLine(`SetSensorTouch(IN_${n});`);
            this.sensorSetup.add(`touch_S${n}`);
        }
        else if (opcode === "legonxt_setupLightSensor" || opcode === "setupLightSensor") {
            const p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            const state = this.getInputValue(block, "STATE", blocks).replace(/"/g, "");
            let n = p.replace("S", ""); if (n === "0") n = "1";
            this.addLine(state === "on" ? `SetSensorLight(IN_${n});` : `SetSensorLowspeed(IN_${n});`);
            this.sensorSetup.add(`light_S${n}`);
        }
        else if (opcode === "legonxt_setupColorSensor" || opcode === "setupColorSensor") {
        ...
        else if (opcode === "legonxt_setupSoundSensor" || opcode === "setupSoundSensor") {
            const port = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            const sensorNum = port.replace("S", "");
            this.addLine(`SetSensorSound(IN_${sensorNum});`);
            this.sensorSetup.add(`sound_${port}`);
        }
        else if (opcode === "legonxt_setupUltrasonicSensor" || opcode === "setupUltrasonicSensor") {
            const p = this.getInputValue(block, "PORT", blocks).replace(/"/g, "");
            let n = p.replace("S", ""); if (n === "0") n = "1";
        ...
        else if (opcode === "legonxt_updateDisplay" || opcode === "updateDisplay") {
        ...
        else if (opcode === "legonxt_captureScreen" || opcode === "captureScreen") {
            this.addLine(`// Screen capture requested`);
        }

        // ===== SPRITE STATE BLOCKS =====
        else if (opcode === "legonxt_spriteSetPosition" || opcode === "spriteSetPosition") {
            const sprite = this.getInputValue(block, "SPRITE", blocks).replace(/"/g, "");
            const x = this.getInputValue(block, "X", blocks);
            const y = this.getInputValue(block, "Y", blocks);
            const safeName = this.sanitizeName(sprite);
        ...
        else if (opcode === "legonxt_spriteSetSize" || opcode === "spriteSetSize") {
            const sprite = this.getInputValue(block, "SPRITE", blocks).replace(/"/g, "");
        ...
        else if (opcode === "legonxt_spriteSetVisible" || opcode === "spriteSetVisible") {
            const sprite = this.getInputValue(block, "SPRITE", blocks).replace(/"/g, "");
        ...
        // ===== STANDARD SCRATCH MOTION BLOCKS =====
            ...
            if (steps.includes("MotorRotationCount")) {
                ...
                this.addLine(`${sprite}.x += (Cos(${sprite}.direction) * ${steps}) / 100;`);
                this.addLine(`${sprite}.y += (Sin(${sprite}.direction) * ${steps}) / 100;`);
            } else {
                // Standard movement
                const sprite = `sprite_${this.activeSpriteName}`;
                this.addLine(`${sprite}.x += (Cos(${sprite}.direction) * ${steps}) / 100;`);
            ...
            const degrees = this.getInputValue(block, "DEGREES", blocks);
            this.addLine(`sprite_${this.activeSpriteName}.direction += ${degrees};`);
        }
        ...
        // ===== CONTROL BLOCKS =====
            ...
            this.addLine(`for(int ${loopVar}=0; ${loopVar}<${times}; ${loopVar}++) {`);
            this.increaseIndent();
        ...
        // ===== EVENT BLOCKS =====
        else if (opcode === "event_broadcast" || opcode === "event_broadcastandwait") {
    ...
    processHatBlock(hatBlock, blocks) {
        ...
        if (opcode === "event_whenflagclicked") {
            ...
            this.logger.success(`  Creating broadcast handler: ${funcName} for "${broadcastName}"`);
    ...
    generateSensorSetup() {
        this.logger.debug(`Generating sensor setup (${this.sensorSetup.size} sensors)`);
    ...
    generateSpriteStateManager() {
        ...
        for (const name of spriteNames) {
            ...
            this.addLine(`sprite_${safeName}.direction = ${state.direction || 90};`); // FIX: Default 90° (right)
            this.addLine(`sprite_${safeName}.size = ${state.size};`);
            this.addLine(`sprite_${safeName}.visible = ${state.visible ? "true" : "false"};`);
        }
        this.decreaseIndent();
        this.addLine("}");
        this.addLine("");
    ...
    transpileProject() {
        ...
        try {
            ...
            for (const target of targets) {
                if (target.variables) {
                    ...
                    for (const varId in target.variables) {
                        ...
                        if (!variable.type || variable.type === '' || variable.type === 'scalar') {
                            ...
                            if (!this.variables.has(varName)) {
                                this.variables.set(varName, sanitized);
                                varCount++;
                                this.logger.log(`    ✓ Variable: "${varName}" → ${sanitized}`);
                            }
                        }
                    }
                }
            ...
            this.logger.log(`  ✓ Generated ${this.variables.size} variable declaration(s)`);
            
            this.generateSensorSetup();
            this.logger.log(`  ✓ Generated sensor setup (${this.sensorSetup.size} sensor(s))`);
            
            this.generateSpriteStateManager();
            this.logger.log(`  ✓ Generated sprite state manager (${Object.keys(this.spriteStates).length} sprite(s))`);
                ...
                for (let i = 0; i < hatBlocks.length; i++) {
                    ...
                    try {
                        ...
                        if (funcName) {
                            ...
                            if (hatBlock.opcode === "event_whenflagclicked") {
                                mainScripts.push(funcName);
                                this.logger.success(`      ✓ Created main script: ${funcName}`);
                            } else {
            ...
            for (let i = 0; i < allFunctions.length; i++) {
                this.logger.log(`  Adding function ${i + 1}/${allFunctions.length}...`);
            ...
            this.logger.log(`  Broadcasts: ${Object.keys(this.broadcastMap).length}`);
  ...
  class NXTPeripheral {
    ...
    _processPacket(telegram) {
      ...
      if (messageType === NXT_OPCODE.REPLY) {
        ...
        logger.debug(`Reply status: 0x${status.toString(16)} (${NXT_ERROR[status] || 'Unknown'})`);
    ...
    async sendTelegram(command, payload = [], requireReply = true, isSystemCmd = false) {
      ...
      logger.debug(`Sending telegram: cmd=0x${command.toString(16)}, len=${length}, reply=${requireReply}`);
  ...
  class LegoNXTExtension {
    ...
    spriteGetX(args) {
      ...
      if (!this.spriteStates[spriteName]) {
        this.spriteStates[spriteName] = { x: 0, y: 0, size: 100, visible: true };
      }
      logger.debug(`Get sprite ${spriteName} x: ${this.spriteStates[spriteName].x}`);
    ...
    spriteGetY(args) {
      ...
      logger.debug(`Get sprite ${spriteName} y: ${this.spriteStates[spriteName].y}`);
    ...
    spriteGetSize(args) {
      ...
      logger.debug(`Get sprite ${spriteName} size: ${this.spriteStates[spriteName].size}`);
    ...
    spriteGetVisible(args) {
      ...
      logger.debug(`Get sprite ${spriteName} visible: ${this.spriteStates[spriteName].visible}`);
    ...
    try {
        ...
        logger.debug(`Open reply: [${Array.from(openReply).map(b => '0x' + b.toString(16)).join(', ')}]`);
