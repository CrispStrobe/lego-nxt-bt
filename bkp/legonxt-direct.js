(function (Scratch) {
  ...
  class NXTPeripheral {
    ...
    async sendSystemCommand(opcode, payload = [], needsReply = true) {
      ...
      console.log(
        `[NXT] Sending system command 0x${opcode.toString(16)}, needsReply: ${needsReply}, packet length: ${telegram.length}`,
      );
    ...
    handleReply(packet) {
      ...
      console.log(
        `[NXT] Reply: type=0x${replyType.toString(16)}, opcode=0x${opcode.toString(16)}, status=0x${status.toString(16)}`,
      );

      // Verify this is a reply
      if (replyType !== NXT_OPCODE.REPLY) {
        console.warn(
          `[NXT] Unexpected reply type: 0x${replyType.toString(16)}`,
        );
        return;
      }
      ...
      for (const [reqId, request] of this.pendingRequests.entries()) {
        if (request.opcode === opcode) {
          ...
          if (status === 0x00) {
            ...
            const errorMsg =
              NXT_ERROR[status] || `Unknown error: 0x${status.toString(16)}`;
            request.reject(
              new Error(`NXT Error: ${errorMsg} (0x${status.toString(16)})`),
            );
          }
          return;
        }
      ...
      console.warn(
        `[NXT] No pending request found for opcode 0x${opcode.toString(16)}`,
      );
    }
    ...
    async getMotorPosition(port) {
      ...
      if (reply && reply.length >= 21) {
        ...
        const position =
          reply[16] | (reply[17] << 8) | (reply[18] << 16) | (reply[19] << 24);
    ...
    async resetMotorPosition(port, relative = false) {
      const portNum = PORT[port];
      await this.sendTelegram(NXT_OPCODE.RESET_POSITION, [
        portNum,
        relative ? 1 : 0,
      ]);
    ...
    async setupSoundSensor(port, adjusted = true) {
      const portNum = PORT[port];
      const sensorType = adjusted
        ? SENSOR_TYPE.SOUND_DBA
    ...
    async getSensorValue(portStr) {
      ...
      try {
        ...
        if (reply && reply.length >= 11) {
          ...
          this.sensorState[portStr].value =
            scaled > 32767 ? scaled - 65536 : scaled;
    ...
    async getSensorValue_old(portStr) {
      ...
      try {
        ...
        console.warn(`[NXT] Read failed:`, e);
      }
      return this.sensorState[portStr].value;
    }
    ...
    async getUltrasonicDistance(portStr) {
      ...
      try {
        ...
        for (let i = 0; i < 8; i++) {
          ...
          const status = await this.sendTelegram(
        ...
        // 3. Read the byte
        if (ready) {
          const data = await this.sendTelegram(
            NXT_OPCODE.LS_READ,
            [portNum],
            true,
          );
      ...
      try {
        for (
          ...
          const payload = [
            ...
            (DISPLAY_OFFSET + offset) & 0xff,
            ((DISPLAY_OFFSET + offset) >> 8) & 0xff,
            sizeToRead & 0xff,
            (sizeToRead >> 8) & 0xff,
          ];
    ...
    screenBufferToBase64() {
      ...
      bmp[offset++] = 0;
      bmp[offset++] = 0;
      bmp[offset++] = 0; // Header size
      bmp[offset++] = width & 0xff;
      bmp[offset++] = (width >> 8) & 0xff;
      bmp[offset++] = (width >> 16) & 0xff;
      bmp[offset++] = (width >> 24) & 0xff;
      bmp[offset++] = height & 0xff;
      bmp[offset++] = (height >> 8) & 0xff;
      bmp[offset++] = (height >> 16) & 0xff;
      bmp[offset++] = (height >> 24) & 0xff;
      bmp[offset++] = 1;
      bmp[offset++] = 0; // Planes
      bmp[offset++] = 1;
      bmp[offset++] = 0; // Bits per pixel
      bmp[offset++] = 0;
      bmp[offset++] = 0;
      bmp[offset++] = 0;
      bmp[offset++] = 0; // Compression
      bmp[offset++] = imageSize & 0xff;
      bmp[offset++] = (imageSize >> 8) & 0xff;
      bmp[offset++] = (imageSize >> 16) & 0xff;
      bmp[offset++] = (imageSize >> 24) & 0xff;
      bmp[offset++] = 0;
      bmp[offset++] = 0;
      bmp[offset++] = 0;
      bmp[offset++] = 0; // X res
